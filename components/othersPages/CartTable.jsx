              ...
              >
                ...
                <td className="px-6 py-4 flex items-center h-full space-x-4 table_tr relative align-middle">
                  ...
                  <div className="flex flex-col service_name">
                    ...
                    <p className="text-sm text-gray-500">{elm.name}</p>
                    <button
                  ...
                  <div className="font-bold flex justify-between items-center border-t border-gray-300 pt-2 mt-auto text-sm w-full">
                    ...
                    <span className="text-blue-700 whitespace-nowrap overflow-hidden text-ellipsis">
                      ...
                      {(() => {
                        ...
                        const orderDetail = cartItems.find(
                          (order) => order.service_id === elm.id
                        );

                        if (orderDetail && orderDetail.total_price) {
                          return Number(orderDetail.total_price).toFixed(2);
                        }

                        const baseTotal = getProductTotal(elm);
                        const wordCountTotal = orderDetail?.fields
                          ? JSON.parse(orderDetail.fields)
                              .filter(
                                (field) =>
                                  field.type === "file" && field.wordCount
                              )
                              .reduce(
                                (total, field) => total + field.wordCount * 0.1,
                                0
                              )
                          : 0;
                ...
                <td className="px-6 py-4 text-center align-middle h-full">
                  <button
                    className={`px-4 py-2 rounded-lg font-medium flex items-center justify-center w-32 mx-auto shadow-sm transition-all duration-200`}
          ...
          >
            ...
            <div className="flex items-start p-4 border-b">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-500 mt-1">{elm.name}</p>
                <p className="font-bold text-lg mt-2">${elm.base_price}</p>
              </div>
            </div>
            ...
            <div className="p-4 bg-gray-50">
              ...
              {/* <div className="flex justify-between items-center pt-3 mt-3 border-t border-gray-300">
                ...
                <span className="font-bold text-blue-700 text-lg">
                  $
                  {(() => {
                    const orderDetail = savedData(elm.id).find(
                      (order) => order.service_id === elm.id
                    );
            ...
            <div className="p-4 bg-white border-t border-gray-200">
              <button
                className={`w-full px-4 py-3 rounded-lg font-medium  flex items-center justify-center shadow-sm transition-all duration-200`}
                onClick={() => handleOpenModal(elm)}
                disabled={loadingItemId === elm.id}
              >
                Check
